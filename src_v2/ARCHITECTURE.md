# LabGuardian v5.2 -- 系统架构与项目介绍

> **版本**: v5.2 (Image-only + YOLO-Pose 引脚定位 + 局部视觉验证 + 统一引脚工具)
> **更新日期**: 2026-03-04
...
[YOLO 元件检测 (9 类, HBB+OBB)]  ------>  [面包板校准 + 网格坐标映射]
   |                                              |
   v                                              v
[Wire 骨架端点精炼]                       [三级引脚定位策略]
[+ 导线颜色分类]                          [1. YOLO-Pose 关键点 (主)]
   |                                       [2. 局部视觉验证 (辅)]
   v                                       [3. 几何候选约束 (回退)]
   |                                              |
   v                                              v
...
|  ImageAnalyzer: 多图分析 + IoU 融合 + 三级引脚定位               |
|  PinHoleVerifier: 局部视觉引脚-孔洞验证 (仅分析元件附近孔洞)    |
|  WireAnalyzer: 骨架化端点检测 + HSV 颜色分类                     |
|  BreadboardCalibrator: 4 级孔洞检测 + Grid RANSAC + Top-K 候选   |
|  pin_utils: 统一引脚属性 + 电气约束评分 + 最佳引脚对选择         |
+------------------------------------------------------------------+
```
...
**引脚延伸先验** (统一管理于 `pin_utils.py`):

| 元件类型 | 延伸比例 (ext_ratio) | 搜索半径倍数 | 最小行跨度 |
|----------|---------------------|-------------|-----------|
| Resistor | 0.18 | 5.0 | 2 |
| LED | 0.18 | 4.0 | 1 |
| Diode | 0.15 | 4.0 | 2 |
| Ceramic_Capacitor | 0.15 | 3.5 | 1 |
| Electrolytic_Capacitor | 0.15 | 3.5 | 1 |
| Transistor | 0.15 | 3.5 | 1 |
| IC | 0.04 | 3.5 | 1 |
| Potentiometer | 0.12 | 3.5 | 1 |
| Wire | 0.02 | 3.0 | 0 |
...
class Detection:
    ...
    # YOLO-Pose 关键点 (当模型升级为 Pose 时启用)
    keypoints: Optional[np.ndarray] = None    # (N, 3) — x, y, conf
    keypoints_conf: Optional[float] = None    # 关键点平均置信度
```

#### 4.1.2 PinHoleVerifier (`pin_hole_detector.py`, ~300 行) — 局部视觉引脚验证

**视觉引脚-孔洞局部验证器** — 当 YOLO-Pose 关键点置信度不足（元件遮挡引脚）时，对估计位置附近的少量孔洞做局部图像分析，判断哪些孔洞被占用。
...
1. 估计位置 → 透视变换到矫正空间
  |
  v
2. 搜索附近孔洞 (search_radius = 孔间距 × search_mult)
   仅取距估计位置 < search_radius 的候选 (通常 10-30 个)
  |
  v
3. 逆透视变换回原始帧 → 提取灰度 Patch (±R 像素)
  |
  v
4. 对比度占用评分 (_compute_occupancy):
   - 中心圆区域 vs 周围环区域的亮度差
   - 空洞: 中心暗, 周围亮 → contrast > 0
   - 被占用: 中心被引脚填充 → contrast ≈ 0 或 < 0
   - Sigmoid 映射: score = 1 / (1 + exp(-(10 - contrast) / 8 × 3))
  |
  v
5. 最佳引脚对选择 (Top-10 × Top-10):
   评分 = 占用分×0.50 + 近距分×0.30 - 约束惩罚×0.20
   约束惩罚复用 pin_utils.score_electrical_constraints()
  |
  v
输出: (逻辑坐标1, 逻辑坐标2) 或 (None, None)
```

**与旧版 PinHoleDetector 的区别**:

| 对比项 | v5.1 PinHoleDetector (旧) | v5.2 PinHoleVerifier (新) |
|--------|--------------------------|--------------------------|
| 分析范围 | 全图 650+ 个孔洞 | 元件附近 10-30 个 |
| 特征维度 | 4 维 (亮度+对比度+梯度+颜色) | 1 维 (中心-周围对比度) |
| 统计模型 | Median+MAD 鲁棒统计 | 简单对比度阈值 |
| 导线阴影过滤 | 117 行聚簇检测 | 已移除 (治标不治本) |
| 电气约束 | 内置重复实现 | 复用 pin_utils 统一模块 |
| 代码量 | ~725 行 | ~300 行 |

#### 4.1.2.1 pin_utils (`pin_utils.py`, ~160 行) — 统一引脚工具模块

集中管理所有引脚定位相关的先验知识和约束评分，消除原 PinHoleDetector 和 ImageAnalyzer 之间的代码重复。
...
# 统一电气约束评分 (面包板规则)
def score_electrical_constraints(loc1, loc2, comp_type) -> float:
    # 同行同侧 = 短路 (+100)
    # 跨中缝同行 (+5)
    # 纯同行非电源轨 (+50)
    # 跨度过大 (+20 + 超出)
...
# 最佳引脚对选择 (几何候选)
def select_best_pin_pair(candidates1, candidates2, comp_type):
    # 评分 = 像素距离之和 + 电气约束惩罚
```
...
#### 4.1.4 ImageAnalyzer (`image_analyzer.py`, ~716 行)
...
输入: 1-3 张 BGR 图片 + conf + imgsz
  ...
  |     未匹配: 加入结果 (新元件, 被遮挡的)
  |
  +-- 4. 三级引脚定位 + 电路建模 (_build_circuit):
  |     策略1 (优先): YOLO-Pose 关键点 (keypoints_conf > 0.6)
  |     策略2 (辅助): 局部视觉验证 → PinHoleVerifier.find_pins_locally()
  |     策略3 (回退): 几何 Top-K 候选 → pin_utils.select_best_pin_pair()
...
```
              ...
              +-----------+-----------+-----------+
              |                       |           |
       [策略1: YOLO-Pose]     [策略2: 局部视觉]  [策略3: 几何候选]
       (keypoints_conf>0.6)  (非Wire, 辅助验证)  (回退, 所有元件)
              |                       |           |
       直接使用关键点坐标   PinHoleVerifier      Top-K 近邻
       (预留, 待Pose模型)   .find_pins_locally() select_best_pin_pair()
              |                       |           |
              +----------++----------++-----------+
                         ||
                   [电路建模]
...
当 YOLO-Pose 模型训练完成后，高置信度关键点将直接提供引脚像素坐标，
无需任何后处理。这是最准确的方法，当前代码已预留接口。

**策略2 — 局部视觉验证**:
对非 Wire 元件，在估计引脚位置附近做局部图像分析。
仅当策略1 无法给出结果时启用。

**策略3 — 几何候选约束匹配**:
从每引脚 K 个候选孔洞中，使用统一电气约束评分选最佳组合。
作为最后的可靠回退。
...
|     +-- 三级引脚定位:                                           |
|     |   策略1: YOLO-Pose 关键点 (预留)                          |
|     |   策略2: 局部视觉验证 (PinHoleVerifier)                   |
|     |   策略3: 几何 Top-K 候选 (pin_utils)                      |
...
│   │
│   ├── vision/                  # 感知层
│   │   ├── detector.py          # YOLOv8 HBB+OBB 双模检测 (9 类, 含 Pose 预留)
│   │   ├── image_analyzer.py    # 图片分析引擎 (多图融合 + 三级引脚定位)
│   │   ├── pin_hole_detector.py # 局部视觉引脚-孔洞验证 (PinHoleVerifier)
│   │   ├── pin_utils.py         # 统一引脚属性 + 电气约束 + 最佳引脚对 (v5.2 新增)
...
### v5.2 (2026-03-04) — 三级引脚定位 + PinHoleVerifier 局部化 + 统一引脚工具

**架构重构**: YOLO-Pose 主导 → 局部视觉验证辅助 → 几何候选回退

- **新建** `vision/pin_utils.py` (~160 行): 统一引脚工具模块
  - `COMPONENT_PIN_PROPS`: 9 类元件引脚物理先验表 (ext_ratio + search_mult + min_span)
  - `score_electrical_constraints()`: 统一电气约束评分 (消除两处重复实现)
  - `select_best_pin_pair()`: 几何候选最佳引脚对选择

- **重写** `vision/pin_hole_detector.py` (725 → 303 行): PinHoleDetector → PinHoleVerifier
  - 从全图扫描 (650+ 孔洞) 改为局部分析 (仅元件附近 10-30 个)
  - 特征从 4 维 (亮度+对比度+梯度+颜色) 简化为 1 维 (中心-周围对比度)
  - 移除 Median+MAD 全局统计建模 (先验假设过强)
  - 移除导线阴影线性聚簇过滤 (117 行, 治标不治本)
  - 电气约束评分复用 pin_utils (消除 ~75 行重复代码)

- **修改** `vision/detector.py`: YOLO-Pose 预留
  - Detection 新增 `keypoints` 和 `keypoints_conf` 字段
  - 引脚延伸参数统一引用 pin_utils.COMPONENT_PIN_PROPS

- **修改** `vision/image_analyzer.py`: 三级引脚定位策略
  - 策略1: YOLO-Pose 关键点 (keypoints_conf > 0.6, 预留)
  - 策略2: 局部视觉验证 (PinHoleVerifier.find_pins_locally)
  - 策略3: 几何 Top-K 候选 (pin_utils.select_best_pin_pair)
  - 移除步骤 3.5 全图占用扫描
  - 移除 _pick_best_pin_pair() (~75 行重复约束逻辑)
...
### 决策 5: 引脚定位采用三级策略 (YOLO-Pose → 局部视觉 → 几何回退)
- **原因**: YOLO-Pose 关键点最准确但依赖高质量训练数据; 元件遮挡时关键点置信度低
- **权衡**: 三级降级确保任何场景都有可用结果; 局部视觉验证仅分析附近孔洞避免全图开销
...
| MAD | Median Absolute Deviation, 中位绝对偏差 (v5.1, 已移除) |
| 占用概率 | 孔洞被引脚插入的视觉评分 (0~1, 基于对比度) |
| 中缝 / Groove | 面包板中央贯穿凹槽, 分隔 a-e 和 f-j 列 |
| YOLO-Pose | YOLO 关键点检测变体, 直接预测引脚坐标 |
| PinHoleVerifier | 局部视觉引脚验证器 (仅分析元件附近孔洞) |
| pin_utils | 统一引脚属性/约束评分工具模块 |

---

*文档版本: v5.2 | 2026-03-04*
