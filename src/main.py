...
# 定义所有可能的搜索路径
search_paths = [
    os.path.join(BASE_DIR, 'runs', 'detect'), # 原有路径
    os.path.join(BASE_DIR, 'runs', 'obb'),    # 可能是 OBB
    os.path.join(os.path.dirname(os.path.dirname(BASE_DIR)), 'runs', 'detect'), # 根目录 runs/detect
    os.path.join(os.path.dirname(os.path.dirname(BASE_DIR)), 'runs', 'obb')     # 根目录 runs/obb
]
  
print(f"Debug: Searching for YOLO models in: {search_paths}")

candidates = []
for p in search_paths:
    # 查找 lab_guardian* 下所有的 best.pt
    found = glob.glob(os.path.join(p, 'lab_guardian*', 'weights', 'best.pt'))
    candidates.extend(found)

if candidates:
    # 0.【新增】用户指定优先使用 lab_guardian_oneshot_v1
    # 我们遍历所有candidates找名为 lab_guardian_oneshot_v1 的
    target_specific = next((c for c in candidates if 'lab_guardian_oneshot_v1' in c), None)
    
    if target_specific:
        YOLO_MODEL_PATH = target_specific
        print(f"Debug: User Requested Demo Model: {YOLO_MODEL_PATH}")
    else:
        # 1. 优先寻找包含 "oneshot" 的模型，因为这些是针对演示优化的
...
class LabGuardianApp:
    def __init__(self, root):
        ...
        self.ar_missing_links = [] # List of tuples [ (pin1_loc, pin2_loc), ... ] for ghost wires
    ...
    def setup_ui(self):
        ...
        # 按钮区 (Prioritize packing at bottom to prevent clipping)
        ...
        ttk.Button(debug_frame, text="💾 Save Lesson Template", command=self.save_lesson_template).pack(fill=tk.X, pady=2)
        ttk.Button(debug_frame, text="📂 Load Lesson Template", command=self.load_lesson_template).pack(fill=tk.X, pady=2)
    ...
    def save_lesson_template(self):
        try:
            file_path = filedialog.asksaveasfilename(
                title="Save Lesson Template",
                defaultextension=".json",
                filetypes=[("LabGuardian Template", "*.json")],
            )
            if not file_path:
                return
            validator.save_reference(file_path)
            self.log(f"✅ Lesson template saved: {os.path.basename(file_path)}")
        except Exception as e:
            self.log(f"Save template error: {e}")

    def load_lesson_template(self):
        try:
            file_path = filedialog.askopenfilename(
                title="Load Lesson Template",
                filetypes=[("LabGuardian Template", "*.json")],
            )
            if not file_path:
                return
            validator.load_reference(file_path)
            self.log(f"✅ Lesson template loaded: {os.path.basename(file_path)}")
        except Exception as e:
            self.log(f"Load template error: {e}")

    def validate_circuit(self):
        if hasattr(self, 'analyzer'):
            self.log("Running validation...")
            # Use the new structured return (dict)
            ...
            # Store missing links for AR visualization in the video loop
            self.ar_missing_links = results['missing_links'] 
            if self.ar_missing_links:
                self.log(f"⚠️ AR Guide activated: {len(self.ar_missing_links)} missing components shown.")
            else:
                 self.log(f"✅ AR Guide: No missing links.")
    ...
    def video_loop(self):
        ...
        while self.is_running:
            ...
            if self.calibration_requested:
                ...
                if len(calib_points) == 4:
                    ...
                    try:
                        ...
                        # --- 新增：孔洞检测（Hole Grid） ---
                        try:
                            hole_count = board_map.detect_holes(warped)
                            if hole_count > 0:
                                self.log(f"✅ Hole grid detected: {hole_count} holes")
                            else:
                                self.log("⚠️ Hole grid not detected (fallback to linear mapping)")
                        except Exception as e:
                            self.log(f"⚠️ Hole detection failed: {e}")
                        ...
                        # (可选) 如果检测到了孔洞，叠加显示部分孔洞点位（用于肉眼验证）
                        try:
                            if getattr(board_map, 'hole_centers', None):
                                # 只画一小部分，避免太密
                                step = max(1, len(board_map.hole_centers) // 120)
                                for (hx, hy) in board_map.hole_centers[::step]:
                                    sx = int(hx * scale_res)
                                    sy = int(hy * scale_res)
                                    cv2.circle(warped_disp, (sx, sy), 1, (0, 255, 255), -1)
                        except Exception:
                            pass
            ...
            if hasattr(self, 'yolo_model'):
                ...
                # 使用动态阈值进行推理
                # 提高 imgsz 有助于细长目标（导线）框更稳定，但会更耗时
                results = self.yolo_model(frame, verbose=False, conf=current_conf, imgsz=960, iou=0.5)
                
                # --- DRAWING DEBUG INFO ---
                # 强制显示当前加载的模型类型
                model_task = getattr(self.yolo_model, 'task', 'detect')
                is_obb_model = (model_task == 'obb')
                debug_color = (0, 255, 0) if is_obb_model else (0, 0, 255)
                # cv2.putText(frame, f"Model: {model_task.upper()} ({'ROTATED' if is_obb_model else 'STD'})", 
                #           (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, debug_color, 2)
                
                if is_obb_model:
                     # OBB 模型不使用默认 plot，以免画出水平框迷惑用户
                    annotated_frame = frame.copy()
                    # 手动绘制 OBB (如果存在)
                    if results[0].obb is not None:
                         # 定义颜色盘 (BGR 格式)
                         # CAP(红), DIODE(绿), LED(蓝), RES(黄), BTN(天蓝), Wire(紫)
                         colors_pool = [(0, 0, 255), (0, 255, 0), (255, 0, 0), (0, 255, 255), (255, 255, 0), (255, 0, 255)]
                         
                         for obb in results[0].obb:
                             cls_id = int(obb.cls[0])
                             color = colors_pool[cls_id % len(colors_pool)]
                             ...
                             if conf > current_conf:
                                 ...
                                 # 获取文字尺寸
                                 (w_txt, h_txt), _ = cv2.getTextSize(label_txt, cv2.FONT_HERSHEY_SIMPLEX, 0.5, 1)
                                 # 在第一个角点处画文字背景
                                 txt_org = tuple(corners[0])
                                 cv2.rectangle(annotated_frame, 
                                             (txt_org[0], txt_org[1] - h_txt - 5), 
                                             (txt_org[0] + w_txt, txt_org[1]), 
                                             color, -1)
                                 
                                 cv2.putText(annotated_frame, label_txt, 
                                            (txt_org[0], txt_org[1] - 5), 
                                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)
                else:
                    annotated_frame = results[0].plot()

                # LOG detected objects for debugging
                # Unified detection list for logging
                log_dets = results[0].boxes if results[0].boxes else (results[0].obb if results[0].obb else [])
                
                if len(log_dets) > 0:
                    det_info = [f"{results[0].names[int(b.cls[0])]}({float(b.conf[0]):.2f})" for b in log_dets]
                ...
                # Check for any valid detections (HBB or OBB)
                has_detections = False
                if results[0].boxes is not None and len(results[0].boxes) > 0:
                    has_detections = True
                elif results[0].obb is not None and len(results[0].obb) > 0:
                    has_detections = True
                
                if board_map.matrix is not None and has_detections:

                    def _wire_endpoints_from_roi(src_bgr, x1i, y1i, x2i, y2i):
                        """Refine wire endpoints within ROI using edges + HoughLinesP.
                        Returns ((xA,yA),(xB,yB)) in full-frame coordinates or None.
                        """
                        h0, w0 = src_bgr.shape[:2]
                        x1c = max(0, min(w0 - 1, int(x1i)))
                        y1c = max(0, min(h0 - 1, int(y1i)))
                        x2c = max(0, min(w0 - 1, int(x2i)))
                        y2c = max(0, min(h0 - 1, int(y2i)))
                        if x2c <= x1c + 5 or y2c <= y1c + 5:
                            return None

                        roi = src_bgr[y1c:y2c, x1c:x2c]
                        ...
                        lines = cv2.HoughLinesP(
                            ...
                            theta=np.pi / 180,
                            threshold=30,
                            minLineLength=max(10, int(0.35 * max(x2c - x1c, y2c - y1c))),
                            maxLineGap=10,
                        )
                        if lines is None:
                            return None
                        ...
                        xA, yA, xB, yB = best
                        p1 = (x1c + int(xA), y1c + int(yA))
                        p2 = (x1c + int(xB), y1c + int(yB))
                        return p1, p2

                    # --- Determine detection source (OBB vs HBB) ---
                    det_list = []
                    is_obb = False
                    if results[0].obb is not None:
                        det_list = results[0].obb
                        is_obb = True
                    elif results[0].boxes is not None:
                        det_list = results[0].boxes

                    # --- Step 1: Gather Raw Detections ---
                    # 临时存储检测到的元件信息，以便进行"位置吸附"后处理
                    raw_components = [] 
                    ...
                    for box in det_list:
                        ...
                        if is_obb:
                            ...
                            # Determine pins based on OBB geometry
                            p0, p1, p2, p3 = corners
                            d01 = np.linalg.norm(p0 - p1)
                            d12 = np.linalg.norm(p1 - p2)
                            
                            if label in ["Wire", "RESISTOR", "DIODE", "Resistor", "LED", "CAPACITOR"]:
                                if d01 < d12:
                                    pin1_pixel = (p0 + p1) / 2
                                    pin2_pixel = (p2 + p3) / 2
                                else:
                                ...
                                pin1_pixel = (cx, y1 + h*0.15)
                                pin2_pixel = (cx, y2 - h*0.15)
                            ...
                            # (Simplified HBB logic for brevity, matches previous behavior)
                            if w > h:
                    ...
                    # --- Step 2: Precise Pin Alignment (Strict Connectivity) ---
                    # 【逻辑修正 - Day 3 v2】
                    # 用户明确指出: "竖向导通，横向不导通"。
                    # 这意味着我们不能随意合并相邻的列 (Strip Index)，因为它们在物理上是严格绝缘的。
                    # 之前的 "Snap Neighboring Rows" 逻辑可能会导致短路，现已移除。
                    
                    # 这里的 pass 之前移除了 loc1/loc2 的计算逻辑，导致 KeyError。
                    # 现在恢复坐标计算，但不进行任何"吸附"操作。
                    ...
                    for c in raw_components:
                        ...
                        def get_logic_loc(px, py):
                            ...
                            wx, wy = dst_point[0][0]
                            return board_map.warped_point_to_logic(wx, wy)

                        c["loc1"] = get_logic_loc(*c["pin1_px"])
                        c["loc2"] = get_logic_loc(*c["pin2_px"])


                    # --- Step 3: Add to Analyzer & Visualize ---
                    for c in raw_components:
                        label = c["label"]
                        loc1 = c["loc1"]
                        loc2 = c["loc2"]
                        pin1_pixel = c["pin1_px"]
                        pin2_pixel = c["pin2_px"]
                        x1,y1,x2,y2 = c["box"]
                        
                        # 如果两个引脚都在有效区域
                        if loc1 and loc2 and loc1[0] != "Groove" and loc2[0] != "Groove":
                            # 添加到电路分析器
                            comp_obj = CircuitComponent(f"{label}", label, loc1, loc2)
                            self.analyzer.add_component(comp_obj)
                ...
                # --- AR Ghost Wire Renderer (新功能：幽灵线导引) ---
                # 当 Validator 发现缺失连接时，画出虚线/箭头提示用户
                if hasattr(self, 'ar_missing_links') and self.ar_missing_links:
                     for pin1_loc, pin2_loc in self.ar_missing_links:
                         try:
                             # 使用孔洞网格/行列中心进行 Logic -> Warped -> Original Pixel 逆映射
                             if board_map.matrix is not None:
                                 inv_matrix = np.linalg.inv(board_map.matrix)

                                 w1 = board_map.logic_to_warped(pin1_loc[0], pin1_loc[1])
                                 w2 = board_map.logic_to_warped(pin2_loc[0], pin2_loc[1])
                                 if w1 is None or w2 is None:
                                     continue

                                 vec1 = np.array([[[w1[0], w1[1]]]], dtype=np.float32)
                                 vec2 = np.array([[[w2[0], w2[1]]]], dtype=np.float32)
                                 dst1 = cv2.perspectiveTransform(vec1, inv_matrix)
                                 dst2 = cv2.perspectiveTransform(vec2, inv_matrix)

                                 p1 = (int(dst1[0][0][0]), int(dst1[0][0][1]))
                                 p2 = (int(dst2[0][0][0]), int(dst2[0][0][1]))
                                 
                                 # 绘制黄色“幽灵线” (Ghost Wire)
                                 cv2.arrowedLine(annotated_frame, p1, p2, (0, 255, 255), 3, tipLength=0.2)
                                 # 画虚线效果 (用圆点模拟)
                                 mid_x, mid_y = (p1[0]+p2[0])//2, (p1[1]+p2[1])//2
                                 cv2.circle(annotated_frame, (p1[0], p1[1]), 5, (0, 255, 255), -1)
                                 cv2.putText(annotated_frame, "MISSING", (mid_x, mid_y), 
                                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
                                            
                         except Exception as ex:
                             print(f"AR Render Error: {ex}")

                # 更新当前检测到的物体
                final_dets = results[0].boxes if results[0].boxes else (results[0].obb if results[0].obb else [])
                if len(final_dets) > 0:
                    # 获取置信度最高的类别
                    cls_id = int(final_dets.cls[0])
